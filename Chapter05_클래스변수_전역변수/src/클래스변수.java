 ...
 *                      클래스 : 다른 데이터형을 모아서 관리할 수 있다 => 가변적이다
 *                      
 ...
 *                         지역변수 : 요청처리를 위한 변수
 *                         매개변수 : 사용자 요청 값이 있는 경우
 *                      => 저장되는 위치 : 스택  
 *      멤버변수 => 메모리에 저장 시점 => new를 이용할 때 따로 생성
 *                 => 사용범위는 클래스 전체/ 다른 클래스에서도 사용이 가능하다
 *      공유변수 => 메모리 저장 시점 => 컴파일시에 한개의 메모리 공간이 생성된다.
 *                  => 사용범위는 클래스 전체/ 다른 클래스에서도 사용이 가능하다 
 * ex1)   class A
 *      {
 *         int a; // 저장이 안된 상태
 ...
 *      }   
 *      
 *   a,c => A aa = new A(); => 저장이 되는 시점
 *     
 *     Stack                  Heap
 *    --aa--           
 *     0x100                                     
 *    ------       메모리주소 -------- new => 동적 메모리 할당 (실행시마다 메모리 생성 => 변수를 묶어주는 역할)
 *                  0x100     ----
 *                              0 -> a   ===> aa.a                        
 *                            ----
 ...
 *      
 *    -- b --
 *       0       => aa.b, bb.b, cc.b, A.b 를 통해 접근이 가능하다 -> 공유가 가능하다
 *    -------                        
 *   ** 멤버변수 사용이 대부분
 *    --------- 지역변수 / 매개변수
 ...
 *       {
 *       	int a;  => 선언만 가능
 *                  => 구현, 제어문, 연산처리, 라이브러리, 메모리 호출이 불가능 
 *                  => 클래스 안에 블록을 잡으면 제어문 연산자 사용가능
 *        }
 *       ------------------------------------------------------------  
 *         지역변수 => 반드시 초기화
 *                                
 ...
 * *** 라이브러리를 사용
 *     --------- 미국에 맞춰져 있음 
 *         |     미국 : 정보제공 중시 / 한국 : 시각적 효과 중시 하는 측면 있음                      
 *    사용하는 프로그램에 맞게 변경해서 사용 => 오버라이딩
 *    Math.random() => 사용할 때 형변환하여 사용해야 함
