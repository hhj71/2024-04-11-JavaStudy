/*
 *  1. replaceAll : 정규식(regex) 이용 => 문자 패턴을 이용해서 찾아서 변경
 *            ** 정규식을 이용하는 또 다른 메소드 => split() -> GPT   
 *            ** 맛있게 맛있는 맛있고... -> 맛있*   
 *  2. substring
 *  3. indexOf
 *  
 */
//단어입력하면 영문으로 출력해주는 프로그램
import java.util.*;
public class 문자열배열_3 {
	...
	public static void main(String[] args) {
		...
		for (String f:과일)
		{
			if (f.contains(fd)) // 지금같은 경우는 startsWith 도 사용가능
			{
		...
		for (String ef:영어과일명)
		{
			if(max<ef.length()) // 문자열의 갯수(길이) => .length()
				max=ef.length();
			if(min>ef.length())
				min=ef.length();
		}
