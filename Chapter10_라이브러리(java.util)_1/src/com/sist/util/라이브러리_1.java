 ...
 * 								 --------- 관련된 클래스 모음 (패키지)
 * 		1. java.lang : import 생략이 가능하다 => 자동 인식
 ...
 * 						--------------------------------------------------------------
 * 				=> StringBuffer : 임시저장장소를 만들어서 문자열 결합시 최적화시킴
 * 						=> append() (메소드): 문자열 결합
 * 				=> Math : 수학연산
 * 				      => random() : 임의 추출
 * 					  => ceil() : 올림
 ...
 *      2. java.util 
 *         ---------
 *         		= 클래스를 사용하기 위해서는 반드시 import를 사용해야 한다.
 *              = 프로그램에 많이 유용하게 사용할 수 있는 클래스의 집합
 *              
 *              = 날짜 관련  
 *              	Date : 기능에 없다 (오라클에 저장된 날짜를 읽어서 저장)
 *              			=> 기능을 추가 : Calendar
 *              
 *              = 문자열을 분해 
 *              StringTokenizer 사용 => 예전에는 split() 사용했었는데 => 결과가 배열 -> 제어하기 어렵다 
 *              --------------
 *              단어별 추출
 *                 
 *              = ★자료구조 => 데이터를 저장해서 쉽게 관리 => 배열 사용 안하게 됨
 *                ------  라이브러리 => 표준화
 ...
 *         브라우저     ===============     자바      =================================================  오라클
 *         ------    				    -------		                                                  -----
 *       화면출력(JavaScript)            | 브라우저에서 사용되는 언어 (브라우저에서 자바는 일반 텍스트) => HTML/XML   | 사용자가 볼 수 있게 데이터 저장 장소
 *      String                           ML => <a> : 태그 => 사용자 정의 		                                             ------------ 데이터베이스
 *      ArrayList                        HTML/XML은 오라클을 연결하는 언어가 아니다 ... (화면 출력)                                 => 저장 / 읽기 / 수정 / 삭제 => CRUD
 *      Integer                        | 자바는 오라클에 있는 데이터를 관리  
 *     ----------
 *     웹에서 사용하는 3대 클래스                     
 */
