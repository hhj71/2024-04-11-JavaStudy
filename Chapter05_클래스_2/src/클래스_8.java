...
class RecipeSystem
{
	Recipe2[] recipes = new Recipe2[40]; // 선언만 가능 => 구현 (예)외부에서 읽기 등..) 못함
	 //초기화 블록 (or 생성자 사용) => 외부에서 데이터를 얻어올 때 / 파일 데이터를 읽을 때 사용
	{
		// 배열에 있는 데이터 초기화
		// class => selector => CSS 구분(태그/class/id) 디자인
		try
		{
			Document doc = Jsoup.connect("https://www.10000recipe.com/recipe/list.html").get(); // doc 안에 있는 모든 데이터가 저장된다.
			// 문서저장 클래스 => HTML을 저장
			//System.out.println(doc.toString());
			Elements title=doc.select("ul.common_sp_list_ul li.common_sp_list_li div.common_sp_caption_tit");
			//System.out.println(title);
			// Element => 태그 <div> --> 이게 여러개 ==> Elements
		...
		}catch(Exception ex) {} // 네트워크, 서버연결, 파일읽기 => 반드시 예외 처리
		// 네트워크 (URL) => 웹서버 연결 후 저장된 데이터 가지고 오기 => 웹 크롤링 
		// 예외처리 => 8장
		// 초기화 블록은 JVM에 위해 자동으로 호출됨
	}
}
public class 클래스_8 {
