 ...
 * 1) 객체지향 프로그램
 *    1. 변수 설정 => 어떤 데이터를 관리할 것인지 설정 (눈에 보인다)
 *    2. 기능 설정 => 메뉴, 검색 등... 메소드...
 * 2) 설계
 ...
 *         변수묶음
 *           1. 변수의 종류
 *               - 객체변수 => new를 이용해서 저장시마다 메모리가 달라진다 (저장공간이 따로 생긴다)
 *                 (=인스턴스 변수 ) (메모리에 저장된 상태) => (객체=인스턴스)
 *               
 *               - 공통변수 => 모든 객체가 동일한 변수를 사용
 *                        => static => 메모리 공간을 한개만 생성이 가능하다
 *           2. 데이터를 보호: 다른 프로그램에서 사용이 안되게 만든다 => 감춘다 (은닉화)
 *              -----------  자체 프로그램에서는 사용이 가능하게 해야 한다. (캡슐화)
 *              -----------  접근지정어              
 *        ----------------
 *          초기화(변수)
 *            1. 명시적인 초기화
 ...
 *                          class A
 *                          {
 *                             int aa; => 0으로 초기화 => 클래스영역에서는 자동으로 초기화된다
 *                             int bb; => 0으로 초기화
 *                           }  
 *                            
 *                           int arr = {1,2};   
 *                                          0x100의 첫번째 값   0x100의 두번째 값
 *                                               arr[0]       arr[1]   => 인덱스를 이용하는 방법 => 메모리크기가 동일
 *                           -- arr --         -------------------------
 *                            0x100        =>       1      |     2     
 *                           ---------         |------------------------
 *                                            0x100  
 *                              ** 데이터를 설계 => 메모리의 크기가 다른것이 있다
 *                             1. 클래스 저장
 *                               A a = new A()
 *                                     ---
 *                               --- a ----
 ...
 *                                                
 *                                              ------------------
 *                              A b = new A() => new => 따로 메모리가 생성이 된다 => 접근하는 범위는 해당 주소에서만 사용이 가능
 *                                    ---
 *                               --- b ----
 *                                 0x200
 *                               ----------  => 0x200 -----------
 ...
 *                                              ------------------   
 *                              
 *                               => new는 사용할 때 마다 따로 메모리 생성(클래스 크기만큼) => 제한 (하드디스크 크기만큼 사용이 가능)
 *                               => new를 이용해서 저장되는 내용
 ...
 *                            메뉴 클릭 => 메소드 (동작)
 *                  => 메소드의 종류
 *                      1) 일반 메소드 (객체메소드) => 객체마다 따로 저장되는 메소드
 *                          [접근지정어] 리턴형 메소드명(매개변수...)
 *                          {
 *                              => 기능 처리
 *                           }
 ...
 *                        
 *                       4) 추상 메소드 : 선언만 되어있는 메소드 => 프로그램에 맞게 구현해서 시용   
 *                           => 윈도우
 *                              버튼 클릭 / 마우스 클릭/ 키보드 입력.........
...
class Card
{
	// 각자 사용되는 변수 => 인스턴스
	int number; // 0으로 초기화됨
	String type; // null로 초기화됨
	// 아직 저장이 안됨 => new를 사용해야 저장이 된다. 
	// 공통으로 사용되는 변수 => 정적 변수 => static 
	static int width = 200, height = 300; //명시적인 초기화 // 저장됨 => static은 컴파일시에 저장됨 
	// 프로그램 종료시까지 메모리가 유지됨 => Heap (프로그래머가 담당하는 메모리 영역)
	// 지역변수, 매개변수는 밤색 (다크모드 연보라색)
	/*
	 *  ----------------------------------------------------------------------------------------------
	 *     종류           메모리 저장 시점          메모리에서 해제      저장위치           사용범위
	 *  ----------------------------------------------------------------------------------------------    
	 *   지역변수           메소드 호출              메소드가 종료       Stack    메소드 안에서만 사용가능
	 *                     => 메모리에 저장        return에 해제
	 *  ----------------------------------------------------------------------------------------------
	 ...
	 *          
	 *          -------
	 *       --------------   => a.aa (. 은 메모리 주소 접근 연산자) => a주소 안에 저장되어 있는 aa 라는 변수에 접근                                                                  
	 *  ----------------------------------------------------------------------------------------------
...
public class 클래스_1 {
	...
	public static void main(String[] args) {
		...
		// 인스턴스 변수 => 메모리가 따로 생성됨 => 각각 영역만 제어 가능
	    card1.number = 7;
	    card1.type = "♥";
