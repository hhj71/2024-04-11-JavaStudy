 ...
 *               ------------ --> 넘겨주는 값 (주소값=> int)
 *               ------------ 동적 메모리 할당 (생성)
 *                            ----------- 실행시마다 메모리 확보(메모리가 따로 생성됨)          
 *                            -----------  메모리 주소가 다르다 => 클래스에서 저장
 *    			int[] arr = new int[5];
 *    			arr= [I@6f2b958e => 메모리 주소
 *                                  연속적인 메모리를 가지고 있다
 *                                
 *                               arr[0] arr[1] arr[2] arr[3] arr[4] => 인덱스를 이용 => 실제로는 일반 변수 사용법과 같음
 *              arr -------------> ------------------------------
 *                                   0  |  0  |  0  |  0  |  0  |
 *                                |------------------------------ 같은 크기의 메모리가 5개 생성
 *                               [I@6f2b958e  
 *                               => 주소를 이용해서 원하는 데이터에 접근하는 변수 => 참조 변수
 *                                  배열/클래스
 *                               => 변수/배열/클래스
...
public class 배열_1 {
	...
	*/
	...
	/*
	 * int[] arr = {10,20,30,40,50}; 
	 * int[] temp = arr; // 실제 주소값이 같아진다. / 얕은 복사 - 같은 영역을 제어 할 수 있게 해준다. 원본에 영향을 줌
	 * temp[0] = 100; 
	 * temp[1] = 200; 
	 * // arr[0]=100 , arr[1]=200
	 ...
	 */
	  ...
	  System.out.println("temp2="+temp2); //temp2=[I@6f2b958e => arr과 같은 주소임을 확인할 수 있다.
	  // 설정 그대로 복사 => 새로운 배열을 생성 => clone => 깊은 복사
	  // 배열은 메모리 주소를 이용한다 => 주소를 대입하면 => 별칭을 준다 같은 메모리를 제어
	  temp[0] = 100; 
	  temp[1] = 200;
	  System.out.println("temp[0]="+temp[0]);
      System.out.println("temp[1]="+temp[1]); 
