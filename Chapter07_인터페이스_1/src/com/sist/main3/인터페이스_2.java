 ...
 *           	=> 추상메소드 : 상속받은 클래스마다 기능은 동일 / 구현 내용이 다른 경우 
 *          -------------------
 *       	자바에서 추상 : 공통적인 
 *       } 
 *       	=> 관련된 클래스를 묶어서 관리
 ...
 *                  3. 데이터 베이스 => 연결, 닫기, SQL 문장  
 *                  4. 웹 프로그램에서 => 요청 받기, 요청 처리, 결과값 전송
 *                => 인터페이스인지 아닌지 확인하는 방법
 *                    -> new를 사용하지 않고 객체를 생성하면 인터페이스 
 *                    -> 추상클래스와 인터페이스는 객체 생성이 불가능하다 -> new를 사용하지 않는다 
 *                => 형식 / 구성요소
 ...
 *                       --------------   -----------
 *                            |                |
 *                            ------------------  IP, PORT => 클래스와 클래스 간의 공통으로 사용되는 변수, 메소드가 있는 경우                  
 * 
 */
