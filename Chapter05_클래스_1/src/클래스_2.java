...
{
	// 멤버 변수 (= 인스턴스 변수, 객체 변수 ....)
	// 인스턴스 => 메모리에 저장된 상태 => new를 이용해서 메모리를 따로 만든 후에 저장해야 한다.
	String name; // null 로 자동 초기화 됨
	int kor, eng, math, total; // 0 로 자동 초기화 됨
	double avg; // 0.0 로 자동 초기화 됨
	char score; // '\0' 로 자동 초기화 됨
	int rank; // 0 로 자동 초기화 됨
