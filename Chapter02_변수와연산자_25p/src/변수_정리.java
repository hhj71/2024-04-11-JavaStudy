 ...
 * -------------------------------------------------------------
 * 정수형
 * 	byte : 1byte (-128~127) => 네트워크 전송시에 주로 사용
 * 	int  : 4byte (-21억 4천 ~ 21억 4천) => 모든 정수는 컴퓨터에서 int로 인식 / 가장 많이 사용되는 정수형 / 웹에서 숫자가 나오면 무조건 int 
 *         ex) 회원정보에서 나이 -> int age / 게시판 게시물 번호 -> int bno, 조회수 -> int hit
 * 	long : 8byte => [최근에 많이 사용 -> 빅데이터 -> AI]  
 * -------------------------------------------------------------
 * 실수형
 * 	double : 8byte  => 평균치, 평점 계산할때 많이 사용.../ 모든 실수는 컴퓨터에서 double로 인식 
 *                      연산처리가 될 때 연산은 같은 데이터형 끼리만 연산이 가능함.
 *                       ex) 10+10.5 -> 10.0+10.5 -> 20.5
 * -------------------------------------------------------------
 * 문자형
 * 	char : 2byte (0~65535) => 문자 1개 저장 -> ' ' / 각 국의 언어 사용이 가능 (Unicode)
 *                            연산시에는 정수로 변경이 된다.  
 *                            ex) 'A'+1 -> 65+1 -> 66
 * -------------------------------------------------------------
 * 논리형
