 ...
 *       ------
 *       정수 
 *         1byte => byte => 파일읽기/업로드/다운로드/네트워크 전송에 주로 사용/ 메모리 공간을 절약할 때 많이 사용 but 요즘은 속도(퍼포먼스)를 더 중요시함 (-128~127)
 ...
 *         8byte => long => 금융권에서 주로 사용                                                           -> 구분 위해 숫자뒤에 L(l) 붙임.
 *         
 *         -> 컴퓨터에 저장
 *                  - 0,1로
 ...
 *          -----------------------------------
 *          부호 비트 1 1 1 1 1 1 1
 *        8bit(1byte) 에서 맨 앞자리는 양수 음수를 표현하는데 쓰임. 0=> 양수, 1 => 음수
 *               
 *       실수 
 *         4byte 
 *         8byte
...
public class 변수_1 {
    ...
    // 문제) 국어 영어 수학 점수를 총점을 계산해서 출력하라 -> 변수를 3개로 잡을 수도 있고 4개로 잡을 수도 있음...
		...
		/*
		 * 문자형
		 * => char: 멀티바이트 => 2byte => Unicode => 0~65535 값으로 문자 표현 가능
		 *   -----  
		 *     | c언어 => 싱글바이트 => 1byte
		 *       => 각 문자마다 번호를 부여 => 아스키코드(ASC)
		 *       => 0~255
		 *       => A -> 65 (아스키코드와 유니코드에서 동일한 값으로 인식됨)
		 *       => a -> 97 
		 *       => '0' -> 48 
		 *       => 표현 방식 => ''로 묶인 단일 문자 리터럴
		 */
