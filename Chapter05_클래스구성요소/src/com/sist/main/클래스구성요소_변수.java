 ...
 *      	=> 구현이 불가능
 *             ---
 *              └> 제어문/연산처리/파일읽기/크롤링/메소드호출....=> 메소드처리, 생성자, 초기화블록 이용
 *          => 선언만 가능하다
 *             ------------
 *             1. 변수 : 한개만 저장이 가능
 *             ---------------------------
 ...
 *      }
 *      -----------------------------------------------------------------------------
 *      변수: 프로그램에 필요한 데이터를 저장하는 공간 
 *           => 프로그램 종료시까지 유지하는 변수 --> 멤버변수 => 자동초기화 (default)
 *      -----------------------------------------------------------------------------
 *      변수 => 초기화 (변수는 초기값이 없는 상태에서는 메모리에 저장이 안됨)
 *         - 생성자 
 *            => 특징 : 일반 메소드와 다르다
 *                     --> 1) 클래스명과 동일
 *                     --> 2) 리턴형이 없다 => void 도 리턴형에 포함
 *                     		예) 
 *                     			class A
 *                     			{
 *                      			void A(){} => 일반 메소드
 ...
 *                            ** 반드시 생성자는 필요하다
 *                            ** 생략이 된 경우에는 자동으로 생성자를 추가 (컴파일러)
 *                            (1) 멤버변수의 초기화 담당
 *                            (2) 클래스를 메모리에 저장할 때 호출되는 메소드
 *                                 ---------------------------------
 *                            (3) 시작과 동시에 처리해야 하는 기능이 있는 경우
 *                                -------------- => 화면 UI, 오라클 연결, 서버 연결, 웹 => 자동로그인 
 *                                    
 *                   --> 5) 생성자 호출
 *                            => new 생성자()
 *                               --- new 없이 호출하면 오류 발생
 ...
 *              *** 초기화 방법
 *              	명시적인 초기화 
 *              		int a = 100; ==> 초기화방법 중에 명시적인 초기화 먼저 되는지 고민 명시적인 초기화 안되면 생성자 & 초기화 블록 이용
 *              	초기화 블록
 *              		int a;
 *              
 *              		{
 ...
 *                 ----  ----  ----   ----- 
 *              
 *            ** 클래스에서는 선언 / 선언과 동시에 초기화 만 가능
 *                 => int a;    /  int a = 100;  
 *                 => int a;
 *                    a=100; ---> 이렇게 하면 오류발생 선언 따로 구현 따로 한거여서 --> 이렇게 쓰고 싶으면 {}안에 넣어야 한다                                         
 *      -----------------------------------------------------------------------------
 ...
 *                   
 *                    => ★ private : 접근 거부 => 다른 클래스에서 사용이 안됨 => 개인정보 => 멤버 변수
 *                    				=> 메소드 (변수의 기능)
 *                    						  --------
 *                                            메모리저장 			/	 	메모리 값 읽기
 *                                            setter     				getter
 *                                            public set변수명()         public get변수명()
 *                                            => 변수는 메모리에 값이 저장 -> 노출 데이터, 노출 메소드 활용 (변경, 상실 방지)
 *                                            => 메소드는 주소값만 저장 => 노출이 없다 (데이터 숨겨짐)
 *                                            ------------------------------------------------------------------ 캡슐화
 *                                            
 ...
 *                       --------- => 특별한 경우를 제외하고는 대부분 public 사용 => 다른 클래스와 연결하기 위해
 ...
 *                        ** 일반 데이터 => 인스턴스
 *                           데이터묶음 => static => 데이터 관리 
 *                             └ (공유되어야 되는 데이터들) 회원목록/ 게시판 목록 => 한개만 가지고 사용                
 *                    
 *                      - 메소드 : 다른 클래스와 연결 
 *                                          | 요청 결과값     | 사용자 요청값
 *                                          |ex)목록을 달라   |
 ...
 *                                                         s부터 e까지 자른 문자열을 달라
 *                        --------------------------------------------------------------------------------
 *                                     o           x    => String trim() => 공백을 제거한 문자열을 달라
 *                                                      => String toUpperCase() => 대문자로 변환한 문자열을 달라
 *                        ---------------------------------------------------------------------------------
 ...
 *                                    ---
 *                                     int/long/double 가능 => 같은 데이터형으로 받는 것을 권장한다. 
 ...
 *                         bbb() 		  ====> A.bbb => 클래스명.메소드명()
 *                       -------
 *                         ddd()          ====> 결과값을 받는다 (return형 o)
 *                       -------                int result = A.ddd()
 *                      ----------                           -------- 리턴형이 있는 경우에는 결과값을 받아서 저장
 *                          A aa = new A();     ---- 
 *         				                          | 리턴에 사용한 데이터형보다 크거나 같다
 *                                                  
 *         						Stack                 Heap
 *         					  --- aa ---
 ...
 *          개념 => 자바 / Spring
 *                ---- 객체지향 프로그램 / 자료구조     
 *         *** 프로그램의 구현 (마음대로) 7개월 안에는 어려울 수 있음
 *         *** 프로그램을 보고 분석/ 수정 이 가능한 것을 최소 목표로... 
 *         
